...
const customAliasesMap = {
  ...
  // Не добавляем сюда просто «Кирилл»: в чате есть Киря Старый и Кирилл (А).
  // Их различают только точные алиасы из personas.json.
  'киря': ['киря', 'кире', 'кирюха', 'кирюхе'],
...
function findByTelegramUsername(username) {
  ...
  return personas.find((persona) =>
    (persona.usernames || []).some((value) => value.replace(/^@/, '').toLowerCase() === normalized) ||
    (persona.aliases || []).some((value) => value.replace(/^@/, '').toLowerCase() === normalized)
  ) || null;
}
...
module.exports = { personas, findByNameOrAlias, findAllMentioned, findByTelegramUsername, listNames };
