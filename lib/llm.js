...
async function generateSummary(persona, history) {
  const raw = await callOpenRouter(
    ...
    [
      ...
      {
        role: 'user',
        content: `Последние сообщения чата:\n${history.slice(-500).join('\n')}\n\nСделай красивую краткую ироничную выжимку: 3–5 коротких пунктов, каждый начинай с «• ». Упомяни главные темы, смешные моменты и один добрый подкол. Не начинай с заголовка «Прожарка», «Краткая выжимка» или имени персоны — заголовок добавит бот. Только готовые пункты.`
      }
    ],
    350
  );
